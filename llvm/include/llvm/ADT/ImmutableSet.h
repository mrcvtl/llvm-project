...
public:
  ...
  /// getMinElement - Find the subtree associated with the lowest ranged
  ///  key value.
  ImutAVLTree *getMinElement() {
    ImutAVLTree *T = this;
    ImutAVLTree *Left = T->getLeft();
    while (Left) {
      T = Left;
      Left = T->getLeft();
    }
...
public:
  ...
  /// Merges two trees using a functor to handle value collisions.
  /// If LeftId is true, the merge assumes that if T1 is empty, the result is
  /// T2. If RightId is true, the merge assumes that if T2 is empty, the result
  /// is T1.
  template <typename MergeFunctor>
  TreeTy *merge(TreeTy *T1, TreeTy *T2, MergeFunctor &Merge, bool LeftId = true,
                bool RightId = true) {
    bool Swapped = false;
    ...
    TreeTy *T = merge_internal(T1, T2, Merge, LeftId, RightId, Swapped);
    markImmutable(T);
    recoverNodes();
    return T;
  }
...
protected:
  ...
  /// Joins two trees with a separator value V.
  ///
  /// pre: The elements of L must be strictly less than V, and V must be
  /// strictly less than the elements of R.
  TreeTy *join(TreeTy *L, value_type_ref V, TreeTy *R) {
    key_type_ref K = ImutInfo::KeyOfValue(V);
    if (L) {
      assert(ImutInfo::isLess(
                 ImutInfo::KeyOfValue(L->getMaxElement()->getValue()), K) &&
             "Elements in L must be strictly less than V");
    }
    if (R) {
      assert(ImutInfo::isLess(
                 K, ImutInfo::KeyOfValue(R->getMinElement()->getValue())) &&
             "V must be strictly less than elements in R");
    }
    if (getHeight(L) > getHeight(R) + 2) {
      TreeTy *LLeft = getLeft(L);
      TreeTy *LRight = getRight(L);
      TreeTy *NewRight = join(LRight, V, R);
      return balanceTree(LLeft, getValue(L), NewRight);
    }
    if (getHeight(R) > getHeight(L) + 2) {
      TreeTy *RLeft = getLeft(R);
      TreeTy *RRight = getRight(R);
      TreeTy *NewLeft = join(L, V, RLeft);
      return balanceTree(NewLeft, getValue(R), RRight);
    }
    return createNode(L, V, R);
  }

  /// Splits the tree T into three parts based on key K:
  /// 1. A tree of elements strictly less than K.
  /// 2. The element with key K (if found).
  /// 3. A tree of elements strictly greater than K.
  std::tuple<TreeTy *, const typename TreeTy::value_type *, TreeTy *>
  split(TreeTy *T, key_type_ref K) {
    if (isEmpty(T))
      return {nullptr, nullptr, nullptr};

    key_type_ref Key = ImutInfo::KeyOfValue(getValue(T));
    if (ImutInfo::isEqual(K, Key)) {
      return {getLeft(T), &getValue(T), getRight(T)};
    }

    if (ImutInfo::isLess(K, Key)) {
      auto [LL, V, LR] = split(getLeft(T), K);
      return {LL, V, join(LR, getValue(T), getRight(T))};
    }

    auto [RL, V, RR] = split(getRight(T), K);
    return {join(getLeft(T), getValue(T), RL), V, RR};
  }

  /// Applies the functor Map to every element in the tree T.
  ///
  /// pre: The functor Map must preserve the relative order of keys.
  /// If the keys are modified, the new keys must satisfy the same
  /// ordering as the original keys. Otherwise, the resulting tree
  /// will be invalid.
  template <typename MapFunctor> TreeTy *map(TreeTy *T, MapFunctor Map) {
    if (isEmpty(T))
      return T;

    TreeTy *NewL = map(getLeft(T), Map);
    TreeTy *NewR = map(getRight(T), Map);

    typename TreeTy::value_type NewV = Map(getValue(T));

    if (NewL == getLeft(T) && NewR == getRight(T) &&
        ImutInfo::isEqual(ImutInfo::KeyOfValue(NewV),
                          ImutInfo::KeyOfValue(getValue(T))) &&
        ImutInfo::isDataEqual(ImutInfo::DataOfValue(NewV),
                              ImutInfo::DataOfValue(getValue(T))))
      return T;

    return createNode(NewL, NewV, NewR);
  }

  template <typename MergeFunctor>
  TreeTy *merge_internal(TreeTy *T1, TreeTy *T2, MergeFunctor &Merge,
                         bool LeftId, bool RightId, bool Swapped) {
    if (isEmpty(T1)) {
      if (LeftId)
        return T2;
      return map(T2, [&](const typename TreeTy::value_type &V) {
        return Swapped ? Merge(&V, nullptr) : Merge(nullptr, &V);
      });
    }
    if (isEmpty(T2)) {
      if (RightId)
        return T1;
      return map(T1, [&](const typename TreeTy::value_type &V) {
        return Swapped ? Merge(nullptr, &V) : Merge(&V, nullptr);
      });
    }
    if (T1 == T2) {
      if (LeftId && RightId)
    ...
    key_type_ref K1 = ImutInfo::KeyOfValue(getValue(T1));
    auto [L2, V2, R2] = split(T2, K1);

    TreeTy *NewL =
        merge_internal(getLeft(T1), L2, Merge, LeftId, RightId, Swapped);
    TreeTy *NewR =
        merge_internal(getRight(T1), R2, Merge, LeftId, RightId, Swapped);

    typename TreeTy::value_type NewVal =
        Swapped ? Merge(V2, &T1->getValue()) : Merge(&T1->getValue(), V2);

    if (NewL == getLeft(T1) && NewR == getRight(T1) &&
        ImutInfo::isEqual(ImutInfo::KeyOfValue(NewVal),
                          ImutInfo::KeyOfValue(getValue(T1))) &&
        ImutInfo::isDataEqual(ImutInfo::DataOfValue(NewVal),
                              ImutInfo::DataOfValue(getValue(T1))))
      return T1;

    return join(NewL, NewVal, NewR);
  }
